 ...
 * expression. In order for this to correctly fit into the AST, the actual
 * nodes are implemented as either an expression or a statement, to ensure that
 * a function expression cannot have a name.
 * 
...
public abstract class JsFunction implements JsNode {
  ...
  public String compile(JsFormatter ws) {
    ...
    return fn + (name == null ? ws.s : ws.ss + name) +
        JsLists.compileParams(params, ws) + ws.s + "{" +
        (body.isEmpty() ? ws.e : ws.ln + JsLists.compile(body, next) +
        ws.idt) + "}";
  }
  
  /**
   * Removes a <code>return</code> statement from the end of the function
