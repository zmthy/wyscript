...
import wyjs.stages.WhileyLexer.Arrow;
import wyjs.stages.WhileyLexer.Bar;
import wyjs.stages.WhileyLexer.Colon;
import wyjs.stages.WhileyLexer.Comma;
import wyjs.stages.WhileyLexer.Comment;
import wyjs.stages.WhileyLexer.Dot;
import wyjs.stages.WhileyLexer.DotDot;
import wyjs.stages.WhileyLexer.ElemOf;
import wyjs.stages.WhileyLexer.EmptySet;
import wyjs.stages.WhileyLexer.Equals;
import wyjs.stages.WhileyLexer.EqualsEquals;
import wyjs.stages.WhileyLexer.GreaterEquals;
import wyjs.stages.WhileyLexer.Identifier;
import wyjs.stages.WhileyLexer.Int;
import wyjs.stages.WhileyLexer.Intersection;
import wyjs.stages.WhileyLexer.Keyword;
import wyjs.stages.WhileyLexer.LeftAngle;
import wyjs.stages.WhileyLexer.LeftBrace;
import wyjs.stages.WhileyLexer.LeftCurly;
import wyjs.stages.WhileyLexer.LeftSquare;
import wyjs.stages.WhileyLexer.LessEquals;
import wyjs.stages.WhileyLexer.LogicalAnd;
import wyjs.stages.WhileyLexer.LogicalOr;
import wyjs.stages.WhileyLexer.Minus;
import wyjs.stages.WhileyLexer.NewLine;
import wyjs.stages.WhileyLexer.NotEquals;
import wyjs.stages.WhileyLexer.Plus;
import wyjs.stages.WhileyLexer.Question;
import wyjs.stages.WhileyLexer.Real;
import wyjs.stages.WhileyLexer.RightAngle;
import wyjs.stages.WhileyLexer.RightBrace;
import wyjs.stages.WhileyLexer.RightCurly;
import wyjs.stages.WhileyLexer.RightSlash;
import wyjs.stages.WhileyLexer.RightSquare;
import wyjs.stages.WhileyLexer.Shreak;
import wyjs.stages.WhileyLexer.Star;
import wyjs.stages.WhileyLexer.Strung;
import wyjs.stages.WhileyLexer.Tabs;
import wyjs.stages.WhileyLexer.Token;
import wyjs.stages.WhileyLexer.Union;
...
public class WhileyParser {
  ...
  private Expr parseConditionExpression() {
    ...
    if (index < tokens.size() && tokens.get(index) instanceof WhileyLexer.None) {
    ...
    if (index < tokens.size() && tokens.get(index) instanceof LessEquals) {
      ...
      return new Expr.BinOp(Expr.BOp.NEQ, lhs, rhs,
        ...
        && tokens.get(index) instanceof WhileyLexer.TypeEquals) {
      return parseTypeEquals(lhs, start);
    } else if (index < tokens.size()
        && tokens.get(index) instanceof WhileyLexer.ElemOf) {
      ...
      return new Expr.BinOp(Expr.BOp.ELEMENTOF, lhs, rhs, sourceAttr(start,
        ...
        && tokens.get(index) instanceof WhileyLexer.SubsetEquals) {
      ...
      return new Expr.BinOp(Expr.BOp.SUBSETEQ, lhs, rhs, sourceAttr(start,
        ...
        && tokens.get(index) instanceof WhileyLexer.Subset) {
    ...
    while (lookahead instanceof LeftSquare || lookahead instanceof Dot
      ...
      if (lookahead instanceof LeftSquare) {
        ...
        if (lookahead instanceof DotDot) {
          ...
          match(RightSquare.class);
          lhs = new Expr.ListAccess(lhs, rhs, sourceAttr(start, index - 1));
        }
      } else {
        match(Dot.class);
  ...
  private boolean isWhiteSpace(Token t) {
    return t instanceof WhileyLexer.NewLine || t instanceof WhileyLexer.Comment
