...
public class ModuleLoader {
  ...
  /**
   * The whiley path is a list of directories which must be searched in
   ...
   * be loaded again.
   */
  private HashMap<ModuleID, Module> moduletable =
      new HashMap<ModuleID, Module>();
   ...
   */
  private HashMap<ModuleID, Skeleton> skeletontable =
      new HashMap<ModuleID, Skeleton>();
  ...
  private static final class Package {
    ...
    /**
     * The modules field contains the names of those modules contained in this
     * package.
     */
     ...
     * identified for a particular package. Each location identifies either a
     * jar file, or a directory. The order of locations found is important ---
     * those which come first have higher priority.
     */
   ...
   */
  private final HashMap<PkgID, Package> packages =
      new HashMap<PkgID, Package>();
   ...
   * 
   * @param pkg The package whose existence we want to check for.
   * 
   ...
   * 
   * @param module A module name without package specifier.
   * @param imports A list of packages to search through. Packages are
   *          searched in order of appearance.
   * @return The resolved package.
   * @throws ModuleNotFoundException if it couldn't resolve the module
   */
  public ModuleID resolve(String name, List<PkgID> imports) throws ResolveError {
   ...
   * @param pkgIngo Information about the including package, in particular
   *          where it can be located on the file system.
   * @return
   */
  private Module loadModule(ModuleID module, Package pkg) throws ResolveError,
      IOException {
        ...
        try {
          ...
          for (Enumeration<JarEntry> e = jf.entries(); e.hasMoreElements();) {
            ...
            if (entryName.endsWith(".wyjs")) {
              ...
              addPackageItem(new PkgID(pkgName.split("\\.")), moduleName,
                  new File(dir));
            }
          }
   ...
   * 
   * @param pkg The name of the module package
   * @param name The name of the module to be added
   * @param location The location of the enclosing package. This is either a
  ...
  public Module readWhileyFile(ModuleID mid, File file) throws IOException {
    ...
    logger.logTimedMessage("Loaded " + filename, System.currentTimeMillis()
        - time);
  ...
  protected Pair<String, Type.Fun> splitDescriptor(String desc) {
    ...
    if (split.length > 2) {
      Type.ProcessName rec =
          (Type.ProcessName) new TypeParser(split[1]).parseType();
  ...
  protected class TypeParser {
    ...
    public Type.NonUnion parseNonUnionType() {
      ...
      case 'N':
        ...
        index++;
        return Type.T_NAMED(new NameID(ModuleID.fromString(pkg), name),
            parseType());
      case '[':
        Type et = parseType();
